...
| `specs/Spec_BoardAnimation_Draft.md` | **§3 Tier A (merge sparkle burst, white-hot bloom, unclipped overshoot) is implemented** (29 Aug 2026, `CellView.swift`/`MergeBoardView.swift`, no schema change) — see §3a for what shipped, including the discovery that the old single `clipShape` was silently clipping the existing scale-up the whole time. **§5 (producer "affordance shimmer") is implemented but UNVERIFIED** (30 Aug 2026, same two files plus `MergeBoardViewModel.swift`, no schema change) — see §5a: its three open design questions (affordability precision, producer scope, non-mammal motif) resolved to strict-cost/family-spawners-only/single-tinted-tuft; built in a remote Linux session with no Xcode or Simulator available, so it has not been compiled, tested, or looked at on screen — do that before treating it as done. Both measured from reference video 26 Aug 2026. `Spec_PartyBoard_Draft.md` remains fully parked pending more reference footage, unrelated to this spec. |
