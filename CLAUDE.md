...
| `specs/Spec_SpendQuotaDailies.md` | D6 — a "Spend N currency" daily-challenge task type, overriding the Alignment Plan's own Warmth-pillar "out" recommendation (decided 18 Aug 2026, adopt anyway). Written cold and design-reviewed 18 Aug 2026 — anchor-sharing, currency scope (both kibble and dog tags), and the standing-quest extension (confirmed out of scope — real separate work, not cheap as first assumed) all resolved. Only §4's numbers still open; not implemented. |
