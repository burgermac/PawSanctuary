...
| `specs/Spec_TaskTrayRedesign_Draft.md` | **Fully implemented 31 Aug–1 Sep 2026** (`11283de`…`6.7`). From a Tasty Travels recording 31 Aug 2026. Closes `Spec_TravelTownReview_Draft.md` §3 (the `TaskStripView` UX debt) with a design none of that spec's three options describe: a 3-wide task tray that collapses to 1-wide by horizontal translation, scrolling freely in a fixed 2-row viewport with a left-edge dot indicator, alongside a horizontal lane carrying **orders** (not quests). **All seven tasks are implemented** — the HUD flattening (`11283de`) — the HUD currency row flattened from three lines to one (level badge · kibble · coins · dog tags · Shop, pinned to `hudPillHeight`), with Rescued/Ambassadors moved to a new `ProfileView` behind `SheetRoute.profile`. That flattening is not cosmetic: §2 measures the board sitting exactly at its own `csW`/`csH` crossover (~50pt both), so the tray has no vertical slack to take and must be paid for out of the header. Confirmed on device — cell pitch measures 50.7pt against a computed `csW` of 51, so the board is width-bound and the ~19pt freed did not reach it. No schema change. **Two open questions block later tasks:** how a countdown renders inside a 40pt tile, where `16h 29m` does not fit (§8.1, blocks 6.2), and tile ordering, since collapse keeps column 3 and therefore decides what is visible at rest (§8.2, blocks 6.4). |
