...
| `specs/Spec_TravelTownReview_Draft.md` | Draft, no code written yet. From a Travel Town recording 29 Aug 2026. Mostly confirms already-shipped work — Travel Town independently runs the same per-order-token + milestone-ladder split as Smile Points/Care Points, three of three reference titles now converge on that shape. Four new deltas recorded, none implemented: orders pay five currencies at once each with its own flight animation (folds into `Spec_BoardAnimation_Draft.md`'s Tier A); the horizontal `TaskStripView` at 15+ cards is flagged as real UX debt (caused actual navigation mistakes this session), no fix chosen; the reference's milestone ladder uses a full-screen celebration takeover PawSanctuary's Care Points claim lacks; and a customer speech-bubble nudge as an order nears completion. Also flags that a full Travel Town starfish-bar payout is unobserved in this capture — the direct measured analogue to Smile Points' modelled bundle size. |
| `specs/Spec_ParallelBoardReview_Draft.md` | Draft, capture only, no code written yet. Fifth in the reference-review series. Deep frame-by-frame dive on the Travel Town "Greek Fest" parallel-board event (`ScreenRecording_08-22-2026 21-33-09_1.MP4`), the reference shape behind the already-shipped `Spec_Phase6b_ParallelBoard.md`. **§1** — the main→parallel switch is a **hard cut** (no wipe/fade/slide/zoom), a ~120 ms staged board fill-in, then an **entry-gated IAP offer** that auto-fires; no exit transition captured. **§2** — measured deltas vs the shipped spec: board is ~7×7 not 5×5; the generator is an **off-grid pedestal tile** with the energy count on it, not an in-grid cell; **energy is a board merge-chain with tap-to-collect (2⚡…100⚡ by tier), not a passive `ParallelBoardEnergy` timer** (the largest delta — §2.1); the reward layer is a 27-set "Event Collection" album paying **main-game energy + gems**, not a single free-lane `ProgressTrack`; many concurrent chains + a wrapped-item layer, not one 5-tier chain; stripped HUD. **§3** — animations: merge choreography reconfirms `Spec_BoardAnimation_Draft.md` §1 (now 3/3 titles); genuinely new are a centred **"Legendary!" tier-name word banner** (§3.2) and the **energy-collect particle-stream** flight (§3.4); the generator spawn-flight arc reconfirms §4 open-question #4 on a second title. **§4.1** flags that this pass did *not* reproduce `Capture_Log.md` 23 Aug finding 5's "second premium currency" — a single shared purple gem on both boards. No design decisions; nothing folded into other specs. |
