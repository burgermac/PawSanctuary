...
| `specs/Spec_Phase6b_RewardLadder.md` | Phase 6b — D8 ("chain offer," renamed to avoid colliding with this codebase's own merge-chain vocabulary). Written cold and design-reviewed 18 Aug 2026. Tasks 3.1–3.5 all implemented and tested (396/396) — IAP + purchase logic, persistence, trigger gate, Shop + task-strip UI, real 6-rung registry content, plus a debug unlock toggle (§5). On-screen click-through still open (see `TODO.md`) — blocked on an unrelated Simulator tap issue, not the feature itself. |
| `specs/Spec_SpendQuotaDailies.md` | D6 — a "Spend N currency" daily-challenge task type, overriding the Alignment Plan's own Warmth-pillar "out" recommendation (decided 18 Aug 2026, adopt anyway). Written cold and fully design-reviewed 18 Aug 2026 — anchor-sharing, currency scope (both kibble and dog tags), §4's numbers (40 kibble / 8 dog tags), and the standing-quest extension (confirmed out of scope) all resolved. Tasks 3.1–3.3 implemented (`QuestGoal.spendCurrency` case, progress pathway, all nine real spend sites wired and tested); 3.4–3.5 still open. |
