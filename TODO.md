...
### Reward Ladder — on-screen verification: Shop section confirmed, two smaller gaps remain (18 Aug 2026)
...
- [ ] **Two smaller gaps remain.** (1) Rung 1 showed no buy button/price — expected, not a bug: this launch went through `simctl launch` directly rather than Xcode's own Run action, so no StoreKit Testing configuration was attached and `storeManager.products` was empty; the code correctly hides the buy button when there's no live `Product` rather than crashing (`if isNext, let product { ... }`). Verifying the actual purchase-and-grant flow on screen needs a real Xcode Run with the `.storekit` config enabled, not a `simctl`-launched build. (2) The `RewardLadderTaskCard` in the horizontal task strip was never visually confirmed — the strip's `ScrollView` didn't respond to `swipe`/`touch_path` gestures aimed at it (unclear why, unlike every other successful tap/swipe this session), so the card was never scrolled into view. Lower priority than the Shop section (the primary surface) and already covered by the same passing unit tests as everything else in this feature.
