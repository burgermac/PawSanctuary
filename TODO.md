...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and two rounds of C done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus four read/write primitives (`item(at:)`, `hasProducer(at:)`, `clearItem(at:)`, `setBubbledAt(_:at:)`); eight functions route through them now instead of indexing `board[...]` directly — the storage/sell trio from round 1, plus the whole bubble-mechanic subsystem (round 2, migrated as one commit since it's a single cohesive feature, not independent candidates). Still open: further Phase C candidates (the superpower group — individually-sized, not a batch) and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A.
