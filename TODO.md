...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and eight rounds of C done (15 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus ten read/write primitives (`item(at:)`, `hasProducer(at:)`, `producer(at:)`, `clearItem(at:)`, `setItem(_:at:)`, `setProducer(_:at:)`, `setUnlocked(_:at:)`, `isEmpty(at:)`, `isUnlocked(at:)`, `setBubbledAt(_:at:)`); forty-one functions route through them now instead of indexing `board[...]` directly. Every Phase C candidate the Appendix named is migrated, including `boardCellTapped`'s "Board interaction" group (round 8) — assessed rather than assumed special, and turned out routine (zero new primitives). Only `apply` was assessed and deliberately left on direct `board` access — its touches are bulk (whole-array assignment, row padding), not single-cell dereferences. **Phase D — the `attemptMergeOrMove` → `MergeResult` rewrite — is now also underway; D0/D1/D2 done, D3 (cleanup) not started, see `specs/BoardStateManager_Phase_D_Plan.md`.** `attemptMergeOrMove` is now 43 lines of pure dispatch (was 144) — every branch (producer merge/swap/move, the eligible-merge pipeline, superpower-piece-spent, item swap, item move) routes through a `MergeResult` case and `apply(_:)`, with `computeMergeOutcome`/`computeProducerOutcome` as the pure, independently-tested decision steps for the two branches that had real decisions to make. Still open: `apply` (`MergeBoardViewModel`'s `GameState`-restore function, by design, not oversight — its `board` touches are bulk, not single-cell) and D3's cleanup pass. Note: `activateProducer` (round 7) has no dedicated commit — a two-session file collision swept its already-tested changes into an unrelated commit (`fce3325`); code confirmed correct, not rewritten, see the plan doc's round 7 entry for the full account. Also flagged along the way: a dead unreachable branch in `selectedItemInfo` (`task_385bed05`, already picked up by the user) and, more seriously, a fresh-install regression where `freshStart()` placed no starter animals or toolbox (`task_86a07a2f`, fixed in `fce3325` with a new regression test).
