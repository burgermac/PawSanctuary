...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and four rounds of C done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus eight read/write primitives (`item(at:)`, `hasProducer(at:)`, `producer(at:)`, `clearItem(at:)`, `setItem(_:at:)`, `isEmpty(at:)`, `isUnlocked(at:)`, `setBubbledAt(_:at:)`); twenty functions route through them now instead of indexing `board[...]` directly — the storage/sell trio from round 1, the bubble-mechanic subsystem (round 2, one commit), the superpower group (round 3 — five functions, each its own commit per the Appendix's "not a batch" call), and the read-only-query group (round 4 — seven functions, five commits: two individual, an unlock-state trio batched, and two more individual). Still open: further Phase C candidates (spawning/producers, item placement/removal, tier/progression, currency, meta/map) and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A. Also flagged along the way: a dead unreachable branch in `selectedItemInfo` (`task_385bed05`).
