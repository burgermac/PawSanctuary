...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A and B done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) now owns `board`/`boardIsFull`/`emptyUnlockedCells`; `MergeBoardViewModel` forwards via a computed passthrough, so none of the ~42 functions touching `board[...]` needed changes. Still open: Phase C (migrate a few self-contained functions one at a time) and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A.
