...
### Back-to-back `persist()` calls can race each other's disk write (found 18 Aug 2026, Reward Ladder Task 3.2)

Writing `RewardLadderPersistenceTests` (a real force-quit/relaunch simulation — `applyPurchase` then a fresh `MergeBoardViewModel().loadGame()`, through the actual disk-backed `GameStore`, not an in-memory `Codable` round-trip) surfaced a genuine race, not something specific to the Reward Ladder: `persist()` calls `GameStore.saveAndSync`, which captures its `GameState` snapshot synchronously but writes it inside a fire-and-forget `Task.detached` ([GameStore.swift:375](PawSanctuary/GameStore.swift:375)). Three `applyPurchase` calls fired with no gap between them landed only **2** rungs on disk after relaunch, not 3 — the detached tasks aren't guaranteed to complete in call order, so a later call's write can finish before an earlier call's, and the earlier (stale) snapshot's write can then land last and clobber the newer one. A single purchase followed by relaunch is unaffected (confirmed passing) — this only bites two or more `persist()` calls close enough together that their writes overlap.

This is distinct from "Background save was fire-and-forget at suspension time" above (already fixed via `saveNow()`) — that was about a single save possibly never running before suspension; this is about multiple saves finishing out of order. Every IAP purchase and most other player actions call `persist()`/`saveAndSync()` the same way, so this isn't Reward-Ladder-specific — it's a pre-existing characteristic of the whole persistence layer, just not previously exercised by a test that fires several saves back-to-back with no gap.

**Why not fixed on the spot:** out of scope for the task that found it (§3.2 was "confirm persistence works," not "fix the persistence layer"), and — like the `claimedTradeIDs` entry above — this is correctness-adjacent (any fix needs to serialize/queue writes or make each write self-consistent against the latest state, not just patch the symptom) rather than mechanical. In practice the window is narrow (two real purchases essentially never land closer together than a StoreKit confirmation sheet allows), but a double-tapped buy button or two rapid actions that each call `persist()` could hit it.

- [ ] Serialize `GameStore`'s writes (e.g. an actor-owned write queue, or only ever let the most recent capture win by checking a monotonic counter/timestamp before writing) so overlapping `persist()` calls can't finish out of order. Needs its own test coverage for the ordering guarantee specifically, not bundled into unrelated feature work.
...
### Reward Ladder — on-screen verification still open, no debug lever to reach it (added 18 Aug 2026)

`specs/Spec_Phase6b_RewardLadder.md` Tasks 3.1–3.5 are fully implemented and tested (393/393) — IAP + purchase logic, persistence, the `isMonetizationUnlocked` trigger gate, the Shop section + task-strip UI, and the real 6-rung `ProgressTrackRegistry` content. But it's never been watched work on screen. Unlike Parallel Board's gap above (blocked on a calendar date), this one is blocked on **player progression**: the feature is gated on `isMonetizationUnlocked` (`hasReachedFirstWall && playerLevel >= 5`), and the existing debug reset (`resetToFreshGame()`) resets *away* from that state, not toward it — there's currently no fast path to a monetization-unlocked test account. A Simulator tap-registration issue hit the same session (a specific button not responding to synthetic taps across several calibrated attempts) also blocked deeper interactive poking, independent of the progression gap.

- [ ] **Either play a test account to level 5 + the first kibble wall the normal way, or add a small `#if DEBUG` toggle** (mirroring `resetToFreshGame()`'s own precedent) that flips `commerce.hasReachedFirstWall = true` for testing — a design-authority call on whether that's worth adding, flagged in the spec's own §5 rather than added unilaterally. Once reachable: confirm `RewardLadderSection` (Shop) and `RewardLadderTaskCard` (task strip) both appear, confirm the purchased/next/locked rung states render as designed, and complete one real purchase to watch `applyPurchase` grant the correct rewards — already unit-tested (`RewardLadderPurchaseTests`/`RewardLadderContentTests`), never watched happen.
