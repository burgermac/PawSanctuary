...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and six rounds of C done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus ten read/write primitives (`item(at:)`, `hasProducer(at:)`, `producer(at:)`, `clearItem(at:)`, `setItem(_:at:)`, `setProducer(_:at:)`, `setUnlocked(_:at:)`, `isEmpty(at:)`, `isUnlocked(at:)`, `setBubbledAt(_:at:)`); twenty-nine functions route through them now instead of indexing `board[...]` directly — the storage/sell trio from round 1, the bubble-mechanic subsystem (round 2, one commit), the superpower group (round 3 — five functions, each its own commit), the read-only-query group (round 4 — seven functions, five commits), the item-placement/removal remainder (round 5 — six functions, five commits), and the remaining small groups (round 6 — `claimAmbassadorQuest`, `collectCurrencyItem`, `buildArea`, `checkTierUnlock`, four commits, no batching). Every Phase C candidate the Appendix named is now migrated except spawning/producers (10 functions, largest, `activateProducer` needs splitting first) and `boardCellTapped`/`apply` (flagged as needing special care, not a routine migration). Still open: that final group, and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A. Also flagged along the way: a dead unreachable branch in `selectedItemInfo` (`task_385bed05`).
