...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Planning done — see `specs/BoardStateManager_Extraction_Plan.md` (13 Aug 2026), not yet executed.** That doc corrects `CODE_HEALTH.md`'s now-stale scope (it lists moving `tickProducers`, which was already deleted in an earlier PERF-03 fix) and lays out a phased approach — extract `board`/`boardIsFull`/`emptyUnlockedCells` as pure state first, following the `KibbleEngine`/`DogTagStore`-style composition pattern already used seven times in this file; leave `attemptMergeOrMove`'s `MergeResult`-dispatch rewrite (the actual high-risk core) as an explicitly separate future sprint. Start execution at Phase A in that doc, not from this line.
