...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and one round of C done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus three read/write primitives (`item(at:)`, `hasProducer(at:)`, `clearItem(at:)`); `sendBoardItemToInventory`, `storeSelectedItemToInventory`, and `sellSelectedAnimal` route through them now instead of indexing `board[...]` directly. Still open: further Phase C rounds (bubble-mechanic group named as the next candidate) and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A.
