...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and three rounds of C done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus eight read/write primitives (`item(at:)`, `hasProducer(at:)`, `clearItem(at:)`, `setBubbledAt(_:at:)`, `setItem(_:at:)`, `isEmpty(at:)`, `isUnlocked(at:)`); thirteen functions route through them now instead of indexing `board[...]` directly — the storage/sell trio from round 1, the bubble-mechanic subsystem (round 2, one commit), and the superpower group (round 3 — `applyAquaticsCurrent`, `applySplitterPiece`, `runStampede`, `handleLeapTap`, `applyPouchPiece`, each its own commit per the Appendix's "not a batch" call). Still open: further Phase C candidates (spawning/producers, item placement/removal, tier/progression, currency, meta/map, read-only queries) and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A.
