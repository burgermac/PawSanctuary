...
- [ ] **Option A (lower risk, heuristic):** cap `claimedTradeIDs` at the most recent N entries (e.g. keep an ordered list or evict oldest on insert past a cap) instead of deriving the exact safe-to-forget condition. Solves the unbounded growth without touching `markClaimed`/confirmation semantics.
- [ ] **Option B (exact, higher risk):** make `markClaimed` return success/failure and only remove the ID from `claimedTradeIDs` once CloudKit actually confirms the status write. Requires care not to reintroduce the double-grant `claimedTradeIDs` exists to prevent — recommend a dedicated pass with trading-specific test coverage, not a bundled edit.
...
- [ ] **PERF-05:** `claimedTradeIDs` grows without bound, serialized/deserialized on every save/load — see "Card trading — `claimedTradeIDs` grows without bound" above. Deliberately deferred (correctness-adjacent, not mechanical).
