...
- [ ] **Extract `BoardStateManager`:** the single largest remaining structural item from `docs/CODE_HEALTH.md`. **Phases A, B, and five rounds of C done (14 Aug 2026) — see `specs/BoardStateManager_Extraction_Plan.md`.** `BoardStateManager` (`PawSanctuary/BoardStateManager.swift`) owns `board`/`boardIsFull`/`emptyUnlockedCells` plus nine read/write primitives (`item(at:)`, `hasProducer(at:)`, `producer(at:)`, `clearItem(at:)`, `setItem(_:at:)`, `setProducer(_:at:)`, `isEmpty(at:)`, `isUnlocked(at:)`, `setBubbledAt(_:at:)`); twenty-five functions route through them now instead of indexing `board[...]` directly — the storage/sell trio from round 1, the bubble-mechanic subsystem (round 2, one commit), the superpower group (round 3 — five functions, each its own commit), the read-only-query group (round 4 — seven functions, five commits), and the item-placement/removal remainder (round 5 — six functions, five commits: three individual, the toolbox pair batched, and one more individual), which together with round 1 closes that whole category. Still open: further Phase C candidates (spawning/producers, tier/progression, currency, meta/map) and Phase D (`attemptMergeOrMove`'s `MergeResult`-dispatch rewrite — the actual high-risk core, explicitly deferred as its own sprint). Pick up at Phase C in that doc, not Phase A. Also flagged along the way: a dead unreachable branch in `selectedItemInfo` (`task_385bed05`).
