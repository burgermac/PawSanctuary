...
**6a — Primitives (implement the Phase 1 stubs)** — draft spec `specs/Spec_Phase6a_Primitives.md`, written cold by Claude Code from this section + the existing `LiveOpsPrimitives.swift` stubs. **Not yet reviewed here** — review before implementation starts, per §2's working method.
- [ ] Scheduler with overlap and priority resolution — a real problem at four-plus concurrent events
- [ ] Token wallet · progress track (parallel free/paid lanes) · reward table · timer service · offer hook · parallel board instance
