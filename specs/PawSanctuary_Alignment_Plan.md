...
- [ ] Pass, free + paid lanes — draft spec `specs/Spec_Phase6b_Pass.md`, written cold by Claude Code at the user's request (no prior design-authority spec existed). **Not yet reviewed.** Flags two things found while drafting: a naming collision with the already-shipped `sanctuaryPass` subscription (renamed this task's IAP product to "Event Pass" to avoid it), and that the single-active-event model (`EventRegistry.currentEvent`) can't actually support D5's "weekly event + continuous 30-day pass running concurrently" — deliberately not fixed in this spec, raise before 6c.
