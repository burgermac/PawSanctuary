...
**Decided 18 Aug 2026 (design authority): adopt** — overriding this section's own "out" recommendation. A "Spend N currency" daily-challenge task type is in scope. No implementation task exists for this yet; whoever picks it up should size the spend quota against the existing daily-challenge reward curve (`QuestCoordinator.swift`) the same way every other tuning number in this plan was modelled, not guessed. Flagged for awareness: the recommendation above still argues *against* this on Warmth-pillar grounds — that reasoning wasn't refuted, it was overridden. Revisit if playtesting or reviews show it reads as out of step with the rest of the game's tone.
...
No implementation task exists for this yet — it needs its own spec, following the same "written cold, design-authority reviewed after" pattern as Pass/Parallel Board, reusing `ProgressTrack`/`RewardTableRegistry`/`OfferHookRegistry` (Phase 6a) plus one new unlock predicate (free lane keyed to "adjacent paid node purchased" instead of "threshold reached") as this section already scoped. Numbers (node prices, payout richness curve, ladder length) are unmodelled placeholders until that spec derives them — the Tasty Travels table above is reference-title data, not PawSanctuary's own tuning.
