...
- [ ] Parallel board — highest revenue, most expensive; the one worth the effort. **Draft spec `specs/Spec_Phase6b_ParallelBoard.md`, written cold by Claude Code at the user's request, same exception made for Pass. Not yet reviewed by the design authority.** Two technical-accuracy/re-scoping passes so far: 15 Aug 2026 (`02d8bb3`) found and fixed three internal problems (a test-event date collision, an underspecified generator mechanic, an overstated drop-in-reuse claim); 16 Aug 2026 (after the concurrent-events prereq and calendar both shipped) re-verified every citation, corrected line numbers that had moved, and found a fifth architectural issue (§0.5) the earlier pass couldn't have known about: simply adding a parallel-board `EventDefinition` to `EventRegistry.allEvents` today would produce an automatic, wrong small-card entry routed to `EventSheetView` (which can't render a board) alongside whatever dedicated full-screen entry point gets built — the concurrent-events work's `ForEach`-over-every-event pattern correctly assumed every event wants the same small-card treatment, because until this event type, all of them did. Needs a decision (an `EventDefinition` kind discriminator, or keeping parallel-board events out of the shared registry entirely) before §3.6's UI task is buildable — flagged, not resolved, same posture as §3.1's still-open `ParallelBoardHosting` question. Also updated §1/§5: the single-active-event gap this spec raised a third time is now fixed (by the same 16 Aug work), which frees the test event's proposed dates (recommended, not mandated: 2026-09-11 → 09-14) to deliberately overlap the real calendar as a three-way concurrency proof, rather than needing to dodge into a gap the way the original 2026-09-04 proposal did. A materially bigger build than its two predecessors regardless: this is the first event type that needs an actual second board, not just a new lane on the existing one. `BoardStateManager` is cleanly reusable as a second instance; `MergeBoardViewModel`'s merge resolution is not, so the plan is a deliberately smaller, standalone merge path; `computeMergeOutcome` (Phase D) is directly reusable for that smaller path's merge decision; `ProducerTile`/`ProducerLevel` are too main-economy-specific to reuse for the event's own generator.
