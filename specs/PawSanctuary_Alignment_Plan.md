...
- [ ] Parallel board — highest revenue, most expensive; the one worth the effort. **Draft spec `specs/Spec_Phase6b_ParallelBoard.md`, written cold by Claude Code at the user's request, same exception made for Pass. Not yet reviewed by the design authority. A technical-accuracy pass has since been done (15 Aug 2026, `02d8bb3`), same as Milestone track's and Pass's own drafts got — found and fixed three real internal problems: §5's original test-event dates collided with the event they were meant to avoid colliding with, §3.3's generator mechanic never specified how the generator cell would actually be distinguished from a normal one, and §2's target-shape table overstated one row as a full drop-in reuse that §3.5 itself correctly qualified. Most of what the spec proposes is still new code with no implementation to check it against, which is what the design-authority review still has to weigh in on.** A materially bigger build than its two predecessors: this is the first event type that needs an actual second board, not just a new lane on the existing one. Flags four things found while drafting: `ParallelBoardHosting`'s protocol is a Phase-1 stub that doesn't match what a real second board needs (§3.1 asks whether to expand it or retire it — genuinely open); `BoardStateManager` turns out to be cleanly reusable as a second instance but `MergeBoardViewModel`'s merge resolution is not, so the plan is a deliberately smaller, standalone merge path, not a second full board orchestrator; `computeMergeOutcome` (Phase D) is directly reusable for that smaller path's merge decision, an unplanned payoff of this session's earlier refactor; and `ProducerTile`/`ProducerLevel` are too main-economy-specific to reuse for the event's own generator. Also raises the single-active-event gap (`EventRegistry.currentEvent`) a third time — flagged in both Milestone track's and Pass's specs already, deferred each time — and says plainly that it's now due before 6c, not after a fourth event has to route around it too.

**6c — Calendar**
- [ ] **Prerequisite, added 15 Aug 2026:** fix the single-active-event model (`EventRegistry.currentEvent`) — flagged three times without being fixed (Milestone track §7, Pass §0, Parallel board §1), and the real calendar below can't honor D5's "concurrently" clause without it. Spec `specs/Spec_Phase6c_ConcurrentEvents.md`, written cold by Claude Code at the user's request, DRAFT — not yet reviewed by the design authority or implemented.
