...
**Decided 18 Aug 2026 (design authority): adopt** — overriding this section's own "out" recommendation. A "Spend N currency" daily-challenge task type is in scope. Whoever picks it up should size the spend quota against the existing daily-challenge reward curve (`QuestCoordinator.swift`) the same way every other tuning number in this plan was modelled, not guessed. Flagged for awareness: the recommendation above still argues *against* this on Warmth-pillar grounds — that reasoning wasn't refuted, it was overridden. Revisit if playtesting or reviews show it reads as out of step with the rest of the game's tone.

Spec drafted the same day: `specs/Spec_SpendQuotaDailies.md` — written cold, design-authority review pending. Proposes a new `QuestGoal.spendCurrency` case reusing the existing daily-challenge generator/stagger machinery almost entirely as-is (kibble only for v1 — no dog-tag economy model exists to size against yet), with one genuinely new piece: every existing daily-challenge goal increments by +1 per discrete event, and this is the first that needs a variable-amount increment, wired through a new chokepoint into the game's two real kibble-spend sites (no shared spend/deduct helper existed anywhere in the codebase before this). Flags one real open design question rather than deciding it silently: because a day's three challenges share one goal "anchor," a spend-kibble day would make *all three* slots spend-kibble, not just one — whether that's fine or needs capping is left for review.
...
**Both decided together, 18 Aug 2026 (design authority):**
- **Adopt, as a variant of the Pass primitive** — matching this section's own recommendation. D6 was decided the same day (adopt, above) — so unlike the "if D6 stays out, the consistent call may be that this stays out too" framing this section itself raised, D6 landing *in* removes the tension the two were meant to be checked against each other for. No inconsistency to resolve.
- **Coercive version — free nodes visible-but-locked**, matching what both reference titles actually ship and the version the §"economics" table above was measured against. The softer parallel-free-track alternative was explicitly rejected, not left as a fallback — it doesn't deliver the "unlock what's already visible" psychology the mechanic depends on, and this section's own read was that it "is just a pass" wearing a different label, not really D8 at all.

**Update, 18 Aug 2026:** `specs/Spec_Phase6b_RewardLadder.md` (the "Reward Ladder," renamed to avoid colliding with this codebase's own merge-chain vocabulary — see that spec's §0) was written cold the same day, design-authority reviewed, and fully implemented and tested (Tasks 3.1–3.5, 396/396 tests). Reused `ProgressTrack` (Phase 6a) alone, not `RewardTableRegistry`/`OfferHookRegistry` as originally guessed here — see that spec's §0 for why (`OfferHookRegistry` turned out to have zero real call sites and doesn't fit a priced, sequenced ladder). Node prices/payout curve/ladder length are no longer unmodelled placeholders — recalibrated against the game's real dog-tag/kibble exchange rate, see that spec's §4.
...
- [x] **1.1** Convert `AdoptionOrder` reward fields → `[OrderReward]` list with a `RewardKind` enum. Update `AdoptionBoard.generateOrder` and the reward distribution path in `MergeBoardViewModel`. `RewardKind` (`AnimalSpecies.swift:768-777`), `OrderReward` (`:785-790`), `AdoptionOrder.rewards: [OrderReward]` (`:808`); `AdoptionBoard.generateOrder` builds it (`AdoptionBoard.swift:66-135`); `MergeBoardViewModel.applyRewards(_ rewards: [OrderReward])` (`MergeBoardViewModel.swift:3109`) is the reward distribution path, named exactly as this item specified.
- [x] **1.2** Add a rider-injection hook so active systems can append to `Order.rewards` at generation time. `OrderRewardProvider` protocol + `OrderRewardRegistry.register`/`unregister`/`riders(playerLevel:)` (`OrderRewardRegistry.swift:14-31`), called directly inside order generation — `rewards.append(contentsOf: OrderRewardRegistry.riders(playerLevel: playerLevel))` (`AdoptionBoard.swift:128`). Load-bearing for every later live-ops event's faucet (Milestone track/Pass/every weekly instance's `EventTokenRiderProvider`).
- [x] **1.3** Add `ChainCategory.currency` and the registry plumbing for it. No chains authored yet — just the category. `ChainCategory.currency` (`ItemChain.swift:30`); the generic `chains(in:)`/`chainIDs(in:)` registry lookups apply to it same as any category. Content followed later, per plan — real kibble/coin currency chains use it (`ItemChain.swift:417`, `:435`), confirming the plumbing actually works, not just compiles.
- [x] **1.4** Add player-state tracking: rolling average purchase value, purchase count, days since last purchase, current wall. **Start recording from first launch — this cannot be backfilled.** `PlayerCommerceState` (`AnimalSpecies.swift:953-977`): `purchaseCount`, `averagePurchaseMicros` (computed), `daysSinceLastPurchase` (computed), `hasReachedFirstWall`/`wallEventsTotal`/`lastWallDate`/`lastWallChainID`/`lastWallTier` for the wall, `firstLaunchDate: Date?` — set exactly once, `if commerce.firstLaunchDate == nil { commerce.firstLaunchDate = Date() }` (`MergeBoardViewModel.swift:888`), matching "cannot be backfilled" literally.
- [x] **1.5** Stub the eight live-ops primitive interfaces (scheduler, token wallet, progress track, reward table, rider injection, parallel board, timer service, offer hook). Interfaces only; implementations in Phase 6. `LiveOpsPrimitives.swift` originally stubbed all eight (its own header: *"Phase 1, Task 1.5 — protocols and value types only, no implementations... so Phase 6 has a target"*) — `EventScheduling`/`TokenWalleting`/`ProgressTracking`/`RewardTabling`/`EventTiming`/`OfferHooking` are still there and now have real Phase 6a implementations; rider injection got no separate stub layer, going straight to the real `OrderRewardRegistry` (1.2) instead since 1.1/1.2 needed it working immediately, not deferred to Phase 6. **One of the eight no longer exists, by a later deliberate decision, not a gap:** `ParallelBoardHosting` was retired 16 Aug 2026 (Phase 6b, Parallel Board spec §3.1) once the real second-board requirements turned out not to fit the shape guessed at in Phase 1 — `ParallelBoardCoordinator` is now held directly by `MergeBoardViewModel`, no protocol indirection. A real, recorded architectural evolution, not evidence this item was skipped.
- [x] **1.6** Schema migration for the above. You're at v24 with an unbroken chain — keep it unbroken. The chain is unbroken and now at v36 (`GameStore.swift`, `currentVersion`). The live-ops primitives' own state got its migration exactly as this item required: `eventTokenWallets`/`progressTracks` added at v29 ("live-ops primitives (Phase 6a)," `GameStore.swift`'s `additiveDefaultsSinceV8`), tested in `PersistenceTests.swift`.

**Definition of done:** game plays identically to today; save/load round-trips; `PersistenceTests` green. **Met.** Corrected 18 Aug 2026: this checklist had sat unchecked since it was written, the same stale-bookkeeping pattern already caught and fixed for Phase 2 (§5) — every later phase built directly on these six primitives and is itself long since shipped and checked off, which isn't possible if Phase 1 genuinely wasn't done. Verified against source per item above, not assumed from context. Full suite green (372/372) at time of this correction.
...
- [x] Pass, free + paid lanes — spec `specs/Spec_Phase6b_Pass.md`, written cold by Claude Code at the user's request (no prior design-authority spec existed). Implemented across 6 commits (`92bdfae`…`064cd02`, merged `ef1d080`) weeks before this checkbox was updated — this entry sat stale as "not yet reviewed" long after the code shipped; implementation raced ahead of the design-authority review the working method calls for. **That review happened 15 Aug 2026** (against the shipped code, not just the spec) and found the implementation faithful to the spec — numbers match §4's table exactly, migration tests present and correct, and the implementer found and closed a real gap in `ProgressTrack` along the way (`isClaimed`, needed because `claimable`'s OR-combined design can't tell which lane is claimed once both render at once). One real bug found and fixed (`e124444`): `applyPurchase` read `activeEvent?.id` only *after* the async StoreKit purchase resolved, so a purchase confirmation straddling the event's end date could charge real money and silently drop the unlock. Fixed by capturing the event at button-tap time instead. Still flags what it always flagged: the naming collision workaround (`sanctuaryPass` vs. the new `eventPass`) and the single-active-event model gap (`EventRegistry.currentEvent`) that D5's concurrent weekly-event-plus-Pass model will eventually need — deliberately not fixed, raise before 6c.
  - [x] **D8 decided 18 Aug 2026: adopt, coercive version** (see D8 above) — now unconditional, no longer "if." Renamed **"Reward Ladder"** to avoid colliding with this codebase's own "chain" vocabulary (merge chains) — spec `specs/Spec_Phase6b_RewardLadder.md`. Turned out smaller a primitive reuse than "same primitive, one predicate" suggested — `ProgressTrack`'s storage carries over but there's no separate faucet to reuse (purchase itself drives progress); `OfferHookRegistry` looked like a fit and wasn't (unused, wrong shape). **Corrected 30 Aug 2026 — this line sat stale as "not yet reviewed... not implemented yet" for over a week after both were done, the same stale-bookkeeping pattern this doc has caught and fixed for itself several times before (Phase 1, Phase 2, D8's own "Blocks" field).** Design-authority-reviewed and fully implemented and tested the same day it was drafted (18 Aug 2026): Tasks 3.1–3.5 (IAP + purchase logic, persistence, trigger gate, Shop + task-strip UI, real 6-rung registry content, a debug unlock toggle), 424/424 tests. Both UI surfaces (`RewardLadderSection` in the Shop, `RewardLadderTaskCard` in the task strip) confirmed rendering correctly on screen 18 Aug 2026, matching design exactly. Only the real StoreKit purchase-and-grant flow still needs a proper Xcode Run to verify on screen (a `simctl`-launched build has no `.storekit` config attached, so the buy button correctly stays hidden) — already covered by passing unit tests either way; tracked in `TODO.md`, not blocking.
- [x] Parallel board — highest revenue, most expensive; the one worth the effort. Spec `specs/Spec_Phase6b_ParallelBoard.md`, written cold by Claude Code at the user's request, same exception made for Pass. Two open architectural forks (§0.1/§3.1 — `ParallelBoardHosting`'s fate; §0.5 — registry placement) and one open persistence question (§3.7 — does the board survive a force-quit?) were all put to the design authority directly rather than decided unilaterally: retire the stub protocol, keep parallel-board events out of `EventRegistry` in their own small separate registry, and persist board+energy (reversing this doc's own draft recommendation of "cheaper, don't persist"). **Implemented across 7 commits (§3.1–§3.7, `595d7b0`…`736e869`) plus a content-authoring pass (§5, `205da50`), all 16 Aug 2026.** `ParallelBoardCoordinator` owns an independent `BoardStateManager`/`ParallelBoardEnergy`/`ProgressTrack`-sharing merge-and-generator board; `attemptMerge` reuses `computeMergeOutcome` (Phase D) at roughly a fifth of `attemptMergeOrMove`'s size; a dedicated full-screen `ParallelBoardView` reuses `CellView` directly (it turned out to already be fully decoupled from `MergeBoardViewModel` — resolved the spec's own open question); board/energy persist via `GameState.parallelBoardState` (schema v35→v36); a real test event ("Second Chances," `second_chances_20260911`, 2026-09-11→09-14) is registered, deliberately overlapping Sanctuary Circle S1 and Playtime Rush's second instance as a three-way concurrency proof. 366/366 tests green. **Design-authority code review complete (18 Aug 2026)** — every shipped file checked against the spec's own claims; core mechanics all matched exactly (including the subtle bits — the swap branch's missing `recalc()`, the load-bearing chain-registration precondition §0.3 flagged). Two real bugs found and fixed: `resetToFreshGame()` (debug-only) didn't reset `activeParallelBoardEvent`, letting a debug reset persist a stale board into a fresh save; and the board view's tap handler left a prior selection active after collecting from the generator, risking an unintended merge on the next tap. A real test-coverage gap was also closed, not just found: `checkEventLifecycle()`'s actual coordinator-lifecycle wiring (creation, `progressTrack` sharing, idempotence, teardown, and the relaunch-restore path) had zero integration coverage — every existing test exercised the coordinator's own methods directly, never the wiring above them. New `ParallelBoardLifecycleTests.swift` (7 tests) closes it, using a new small injectable-date seam (`loadGame(at:)`) to prove the relaunch-restore path against the real shipped `loadGame()`, not a reimplementation — same precedent `checkEventLifecycle(at:)`/`ParallelBoardEventRegistry.activeEvent(at:)` already set. 441/441 tests green. Full account in the spec's own end-of-file update. **One item still genuinely open, not just unreviewed:** the spec's own "verified on screen" acceptance item — the real test event doesn't open until 2026-09-11, so the actual tap-generator-merge-watch-progress flow has never been watched happen on a running instance of the app (a same-day attempt to shortcut this via the iOS Simulator's clock didn't succeed and was abandoned rather than forced; see `TODO.md` and the spec's own §5 status note). A one-time scheduled reminder exists for that date. Also worth noting: `ParallelBoardEventRegistry.allEvents` has grown to 3 seasonal instances (one per Sanctuary Circle season, decided 17 Aug 2026) since this bullet was last updated — the "a real test event" framing above describes only the first of those.
