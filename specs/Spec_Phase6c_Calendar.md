...
- **D8 (chain offer):** decided 18 Aug 2026 — adopt, coercive version (Alignment Plan §3) — but no spec or implementation exists yet, so it still has nothing to schedule. **Not included in this calendar.** Only the two event types with a decided, shipped, tested mechanic — Milestone track and Pass — appear below; D8, like Parallel Board was before it shipped, is a separate follow-up to layer in once it exists.
...
- ~~Parallel Board content — Parallel Board itself isn't implemented; nothing here can schedule it. Revisit this calendar once it ships.~~ **Done, 17 Aug 2026.** Parallel Board shipped in full (`specs/Spec_Phase6b_ParallelBoard.md` §0–§5, 16 Aug 2026) — this is the revisit this line called for. Cadence decided by the design authority: **monthly, one instance per Sanctuary Circle season** (3 total, not the weekly track's cadence), read as matching the Alignment Plan's framing of Parallel Board as "highest revenue, most expensive" — a premium/occasional event, not a fourth routine weekly beat. Three "Second Chances" instances added to `ParallelBoardEventRegistry.allEvents` (`ParallelBoardEvents.swift`, a registry deliberately separate from `EventRegistry` per §0.5 of the Parallel Board spec, so nothing in *this* file changed): `second_chances_20260911` (Season 1 — the same instance §5 of the Parallel Board spec authored as its screen-verification test event, now doing double duty rather than being duplicated), `second_chances_20261011` (Season 2), `second_chances_20261110` (Season 3). Same "reward-curve reuse, not new numbers" discipline this spec's own §2.2 established: all three reuse the "Second Chances" name, chain, and 8-milestone progress-track shape verbatim, re-keyed per event ID only — the identical reasoning §2.2/§6 already gave for Sanctuary Circle reusing one name/table across its own three seasons. 6 new tests (`ParallelBoardCalendarLayerTests`, `PawSanctuaryTests/ParallelBoardEventsTests.swift`): all three exist and no extras, each runs exactly 3 days, each falls entirely within its paired season, no two overlap each other, all three share the same name/chain, and all three have an identical-shaped progress track. 372/372 tests green. Genuinely additive — zero changes to `EventRegistry`, `checkEventLifecycle()`, or any UI call site, matching this spec's own §7 posture below.
- D8 (chain offer) content — decided 18 Aug 2026 (adopt, coercive version, Alignment Plan §3), but still out of scope here: no spec or implementation exists yet to schedule content for, the same posture Parallel Board had in this section until it shipped.
