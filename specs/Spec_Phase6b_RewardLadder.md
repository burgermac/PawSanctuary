...
## 5. Task — one screen-verifiable test instance — content done, debug lever added, click-through still open
...
**Debug toggle added, design-authority confirmed 18 Aug 2026:** `MergeBoardViewModel.unlockMonetizationForTesting()` (`#if DEBUG`) plus a HUD button (`MergeBoardView.swift`) in the same slot the real Shop button occupies once unlocked — not inside `ShopView` itself, since that's only reachable once `isMonetizationUnlocked` is already true (a toggle placed there would only ever fire as a no-op). Adding this also surfaced a real, separate, previously-undiscovered project issue — `SWIFT_ACTIVE_COMPILATION_CONDITIONS` was never set anywhere in `project.pbxproj`, so `#if DEBUG` had evaluated false in every command-line-built binary of this app, ever, silently dropping every debug affordance including `resetToFreshGame()`'s own button. Fixed at the project level; see `TODO.md` for the full account. Confirmed via Simulator screenshot: the "Unlock" button now correctly renders on a fresh, level-1 account.

- [ ] **Real screen verification is still open** — reachable now, but not completed this session. A Simulator tap-registration issue (the daily-login modal's own `Claim!` button, unrelated to the debug toggle) blocked getting past the very first screen. Once past it: tap "Unlock," confirm the Shop's `RewardLadderSection` and the task-strip `RewardLadderTaskCard` both appear, confirm the ladder view's layout (purchased/next/locked states) matches §3.4's design, and complete a real rung purchase to confirm `applyPurchase` grants the right rewards on screen (unit-tested already in `RewardLadderPurchaseTests`/`RewardLadderContentTests`, but never watched happen). See `TODO.md` for the current status.
