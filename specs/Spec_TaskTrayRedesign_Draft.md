...
>
> **Parallel Board verified 11 Sep 2026**, once `second_chances_20260911`'s window actually opened. The tile renders as designed — `arrow.triangle.2.circlepath`, its blue tint, the progress bar in the status slot rather than a countdown (exactly the §5-row-11 / bar-wins-the-slot rule above) — and its `showParallelBoard` tap route opens `ParallelBoardView` full-screen. Note for anyone reading row 11's table: the "`ParallelBoardTaskCard`" column names the pre-redesign surface this replaced; the live code is `TaskTrayView.parallelBoardTile`. **Loyalty remains unreached.**
