...
- [ ] `ParallelBoardCoordinator` owns an independent `BoardStateManager` — verify by placing items on both boards simultaneously and confirming neither's state leaks into the other — **structurally true since §3.3 (each coordinator instance gets its own `BoardStateManager()`); the "simultaneously, against the real main board" verification needs `MergeBoardViewModel` to actually hold one, still blocked on §3.7**
- [ ] `ParallelBoardEnergy` regenerates on the existing foreground timer tick, caps correctly, and `spend(_:)` correctly rejects an insufficient balance — **regen/cap/spend behavior done and unit-tested (§3.2, 16 Aug 2026); wiring onto the actual foreground timer tick is still open, blocked on §3.7's coordinator existing**
...
- [ ] A completed top-tier item awards the event's `ProgressTrack`, visible immediately in whatever UI 3.6 builds — **the award itself is done and unit-tested (§3.5, 16 Aug 2026); "visible immediately" needs the UI, still blocked on §3.6**
