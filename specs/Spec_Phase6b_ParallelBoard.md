...
**Design-authority code review complete (18 Aug 2026)**, the review this doc's own header always said hadn't happened yet — the same pass Milestone track and Pass already had. Read every shipped file against every claim in this doc, section by section: `ParallelBoardCoordinator`/`ParallelBoardEnergy`/`ParallelBoardEvents`/`ParallelBoardView`, the `MergeBoardViewModel` lifecycle wiring, the `GameStore` v36 migration, the "Second Chances" chain/progress-track content (now expanded to 3 seasonal instances, one per Sanctuary Circle season, decided 17 Aug 2026 — not just the one test event this doc describes). All the core mechanics matched this spec exactly, including the subtle parts — the swap branch's missing `recalc()` call is correct, not a bug, since a swap between two occupied cells never changes `emptyUnlockedCells`; the load-bearing `ContentRegistry` chain registration this doc's §0.3 flagged as easy to silently skip was in fact done.

Two real bugs found and fixed, both outside this spec's own text (found by reasoning about what a debug tool and a UI interaction do to state this spec introduced, not by re-reading the spec itself):
1. `resetToFreshGame()` (`#if DEBUG`) resets every other live-ops subsystem but hadn't been updated to also clear `activeParallelBoardEvent` — a debug reset while an event was active left the stale coordinator's board/energy in memory, and the reset's own `save()` call would persist that stale state into the otherwise-fresh save. Fixed: also nils `activeParallelBoardEvent`/`pendingParallelBoardRestore`.
2. `ParallelBoardView.handleTap`: tapping the (empty) generator cell while another cell was already selected collected from the generator but left the old selection active — the next tap could silently attempt an unintended merge with a selection the player had likely forgotten about. Fixed: collecting now clears `selectedCell`.

One real test-coverage gap found and closed, not a bug: every file's own logic was unit-tested in isolation, but `MergeBoardViewModel.checkEventLifecycle()`'s actual integration wiring — coordinator creation with the right `chainID`/shared `progressTrack`, idempotence across repeated calls, teardown when the event ends, and above all the relaunch-restore path (`apply(_:)` → `pendingParallelBoardRestore` → `checkEventLifecycle` consuming it) — had zero coverage anywhere. Proving the relaunch-restore path against the *real* shipped `loadGame()` (not a reimplementation) required a small injectable-date seam, `loadGame(at date: Date = Date())`, threaded only to its internal `checkEventLifecycle` call — the same kind of seam `checkEventLifecycle` and `ParallelBoardEventRegistry.activeEvent` already had for the identical reason. New file: `ParallelBoardLifecycleTests.swift`, 7 tests, all passing against the real `second_chances_20260911` window — including a positive case (an item placed via the generator survives a same-window relaunch, energy spent stays spent) and a negative case (a stale save from an ended event never seeds a later, different event's board). 441/441 tests green suite-wide.

**Confirms the acceptance checklist below is accurate as written** — the review found no reason to change any `[x]`, and the one remaining `[ ]` (on-screen verification) is exactly as open as this doc already said, for the same real-date reason, unrelated to anything this review checked.
...
- [x] **Verified on screen — done 11 Sep 2026, on `second_chances_20260911`'s real opening day, no clock manipulation needed.** iPhone 17 Pro Simulator (iOS 26.5), Debug build, fresh save. The full sequence ran end to end: the "Second Chances" tile appears in the task tray (icon `arrow.triangle.2.circlepath`, blue tint) → tapping it opens `ParallelBoardView` full-screen showing the 5×5 board, the generator cell's blue ring + `bolt.circle.fill` badge at (0,0), and `30/30` energy → each generator tap places one base-tier "Stray" on a random empty cell and debits exactly `parallelBoardGeneratorCost` (2), watched down 30→28→→0 across 15 taps → tap-select-then-tap-target merges advance the tier through the whole authored chain, each rendering its own art (Stray → Rescued → Cared For → Thriving → **Forever Home**, the last carrying its `checkmark.seal.fill` badge) → and completing that top-tier item advanced the header progress bar from empty to a visible orange segment in the same frame. **The credit is exactly `parallelBoardTokensPerCompletion`:** `progressTracks["second_chances_20260911"].progress == 10` read back out of `gameState.json`, against this table's 160 top threshold — 6.25% of the bar, which is what the rendered segment measures. Energy regen was also observed live on the foreground tick (0→1→2 while the board was open), which the unit tests only prove in isolation.
- [x] **Force-quit / relaunch mid-event restores the board (same session).** With a deliberately mixed board standing (tier 3 at (2,1), tier 0 at (4,0), tier 1 at (4,2), tier 2 at (4,4), energy 1/30), backgrounding flushed `persistNow()` and `gameState.json` (schema v41) carried `parallelBoardState` with `eventID: second_chances_20260911` and all four cells at their exact tiers; `simctl terminate` + `simctl launch` against the **same** data container brought the app back with `checkEventLifecycle()` reconstructing the coordinator and `restore(from:)` replaying board *and* energy identically — not a reset board. (Relaunch via a reinstall would have moved the app to a new container and orphaned the save, which is why this was driven through `simctl` rather than a fresh install.)
