...
### 6c. Step 3a IMPLEMENTED (12 Sep 2026) — event lifecycle and forfeit on close

**3a is a step this §6 did not have.** The landing order went from the accumulator (step 2) straight to IAP (step 3), but nothing ever *created* a `KibbleDriveState`: step 2 shipped a field only tests could populate, and step 6's tray tile would have had nothing to render. Inserted as 3a rather than folded into step 3, because purchase logic and lifecycle fail differently and a combined commit would not have been separately revertible.

New `KibbleDriveEvents.swift` (`KibbleDriveEventDefinition` + `KibbleDriveRegistry`) and a third block in `checkEventLifecycle`, mirroring the Parallel Board's shape for the reason `ParallelBoardEvents.swift` gives: a Drive never participates in the rider-provider bookkeeping that registry does for ordinary events, so folding it into `EventRegistry.allEvents` would mean teaching every consumer of `activeEvents` to skip it. Same **launch-only posture** as both blocks above it — a Drive that ends while the app is open is torn down on the next launch, not by a mid-session poll. 608/608 green.

**§7's open question 4 is resolved: forfeit on close.** Decided by Tim, 12 Sep 2026. When the window closes the state is dropped outright — unclaimed rungs are lost, points do not roll over. Worth being explicit that this is the **opposite** posture to the Reward Ladder, which deliberately never expires (`Spec_Phase6b_RewardLadder.md` §0). The two differ because the Ladder sells rungs directly where the Drive sells *a window in which to earn them*, and a window that does not close is not a window. That is a deliberate divergence, recorded here so the next reader does not "fix" one to match the other.

Forfeit also turns out to carry its weight structurally: because state never survives the gap between two Drives, the `eventID` comparison that decides whether to create a fresh `KibbleDriveState` is *sufficient* rather than merely convenient. Stale points cannot leak into a later Drive, and two tests pin exactly that.

**The schedule lands here, the ladder does not.** A Drive needs a window before its lifecycle can be written or watched, so 3a carries §4's cadence — 3-day windows 28 days apart — and step 4 still owns §3.3's rung table. Two judgement calls in it that are Tim's to overturn cheaply:

1. **The first window is live on the day this was written** (2026-09-11 → 09-14), so lifecycle could be verified on screen rather than only against an injected date. Exactly the choice `Spec_Phase6b_ParallelBoard.md` §5 made for `second_chances_20260911`, which `ParallelBoardEvents.swift` records doing double duty as both the screen-verification event and Season 1's real content. It is the one date here chosen for testability rather than for the calendar.
2. **It therefore overlaps that Second Chances window exactly.** Allowed — §4 expects concurrency and `Spec_ParallelBoardReview_Draft.md` §2.5 records the reference running 3+ surfaces at once — but worth a deliberate look in step 4.

**A rule I invented and then removed, recorded because it nearly shipped.** The first draft of this file asserted that a Drive window must never overlap a Parallel Board one, on the reasoning that two premium offers landing on the same days compete for a single purchase with no upside. That is a plausible commercial instinct; it is also **not in this spec and contradicts §4**, and I had written it as a *test* — so a schedule written to the spec failed against an unsourced rule masquerading as a requirement. The rule is gone and the test now asserts the permission instead of the prohibition. If the instinct is right it belongs in the spec first, decided rather than smuggled in through a test.

**Two of step 2's tests were superseded and are documented in place rather than deleted quietly.** `testDrivePointsSurviveAForceQuitAndRelaunch` and `testNoDriveRoundTripsAsNoDrive` seeded a `kibbleDrive` whose `eventID` was not in the registry and asserted it survived `loadGame()`. Once lifecycle exists that is *impossible by construction* — `checkEventLifecycle` is the sole author of the field, and a Drive with an unscheduled ID not surviving a load is precisely the stale-state leak forfeit-on-close exists to prevent. Their real coverage moved to `KibbleDriveLifecycleTests`, against genuine registry events and fixed dates. A note sits where they were, because "a test disappeared" and "a test was superseded" look identical in a diff.

**`KibbleDriveLifecycleTests.swift`, 12 tests, every date fixed and permanent** — taken from the registry's own schedule, never from `Date()`. Lifecycle tests that read wall-clock time rot the moment the window they depended on closes; `TODO.md`'s resolved test-rot entry is that exact failure demonstrated on the Parallel Board's equivalent. Coverage: window length and cadence (§4's rarity argument as an assertion, so a later retune that tightens spacing fails loudly rather than quietly opening the faucet), distinct IDs, creation, teardown, forfeit, no-leak-into-the-next-Drive, mid-window relaunch, and one end-to-end case that crosses the weekly boundary *inside* a live window — which exercises §1's trap in its real production order, since `loadGame` runs `checkWeeklyGoalReset()` before `checkEventLifecycle`. Step 2 could not reach that ordering.

**Verified on screen, and this time nothing was seeded.** The first window being live meant the app created the Drive itself: a launch against the real save wrote `kibbleDrive = {eventID: "kibble_drive_20260911", points: 0, purchased: false}` where the field had been absent. Then ordinary play — spawning pups from the Antique Dog House and merging them, which fulfils orders — moved `carePointsThisWeek` **9 → 11** and `kibbleDrive.points` **0 → 2** together, the same +2 to both, into a Drive no test harness had touched. `purchased` stayed `false`, so accrue-always holds against lifecycle-created state and not just seeded state. The Drive left in the save is genuine production content, so unlike step 2's check there was nothing to clean up.

**Not verifiable on screen: forfeit on close.** It needs the clock past 2026-09-14, and iOS Simulators expose no date override — `TODO.md` records that being run to ground definitively, Settings and `simctl` both. It is covered by four fixed-date tests instead, and is the one behaviour in 3a that has been proven only in test.
