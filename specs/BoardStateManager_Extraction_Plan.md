...
**Status: Phase A, B, and eight rounds of C complete; Phase D underway (15 Aug 2026).** `BoardStateManager` exists (`PawSanctuary/BoardStateManager.swift`), owns `board`/`boardIsFull`/`emptyUnlockedCells`, and ten read/write primitives (`item(at:)`, `hasProducer(at:)`, `producer(at:)`, `clearItem(at:)`, `setItem(_:at:)`, `setProducer(_:at:)`, `setUnlocked(_:at:)`, `isEmpty(at:)`, `isUnlocked(at:)`, `setBubbledAt(_:at:)`) used by forty-one migrated functions — every Phase C candidate the Appendix named, including `boardCellTapped` and its "Board interaction" group, which turned out to be a routine migration after all (round 8). Only `apply` was assessed and deliberately left on direct `board` access — its touches are bulk (whole-array assignment, row padding), not single-cell. **Phase D (the `attemptMergeOrMove` → `MergeResult` rewrite) has its own planning doc, `specs/BoardStateManager_Phase_D_Plan.md` — D0/D1/D2 are done there, D3 (cleanup) is not started.** `attemptMergeOrMove` is now 43 lines of pure dispatch (was 144). This is still a "solid progress, not finished" checkpoint, not a completed extraction. This doc originally superseded `docs/CODE_HEALTH.md`'s scope for this item, which had partly gone stale (see §1).
...
- **Phase D — the `attemptMergeOrMove` → `MergeResult` rewrite. D0/D1/D2 done, D3 not started; see `specs/BoardStateManager_Phase_D_Plan.md` for the full account, not duplicated here.** This was the actual high-risk core of `docs/CODE_HEALTH.md`'s original vision, and got its own planning pass before any code changed, per the note below (characterization tests first, then a pure decision step, then an incremental branch-by-branch cutover — not one big rewrite). `attemptMergeOrMove` is now pure dispatch: 43 lines, down from 144, with zero inline `board[...]` writes.
