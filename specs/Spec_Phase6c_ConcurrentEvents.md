...
**Status (16 Aug 2026): §3.1–§3.3 done, §3.4–§3.5 not started.** `EventRegistry.activeEvents` added (commit `b023431`), routed through `EventScheduler`. `MergeBoardViewModel.activeEvent` renamed to `activeEvents: [EventDefinition]` (commit `19404b1`) — the two UI sites still read `.activeEvents.first`, preserving today's single-card behavior until §3.4 replaces them with a real `ForEach`; the Event Pass purchase-flow fallback reads `activeEvents.first?.id`; `claimEventMilestone` (dead `EventProgress` system, out of scope per §6) reads `EventRegistry.currentEvent` directly, untouched by the rename. **§3.3 — the load-bearing fix — is now done too:** `checkEventLifecycle()`'s single `activeEventRiderProvider: EventTokenRiderProvider?` is now `activeEventRiderProviders: [String: EventTokenRiderProvider]`, diffed against `EventRegistry.activeEvents` on every check (unregister providers for IDs that dropped out, register providers for IDs that newly appeared, leave unchanged ones alone) — matches the spec's own §3.3 sketch. `OrderRewardRegistry` needed no change; it already `flatMap`s every registered provider. Two events active simultaneously can now each register and keep their own rider — previously only one could ever hold the slot, the other would silently earn nothing. No behavior change today: still zero real overlapping events in the registry, so this is provably inert until §4's test event lands. Full suite verified at 285/285 green, 0 regressions after both §3.2 and §3.3. **Next: §3.4** (UI: task strip `ForEach`, `TaskSheet.event` gains an associated event-ID, sheet routing looks up the tapped event) **then §4** (the real overlapping test event that proves all of this on screen).
...
- [ ] `EventRegistry.activeEvents` returns every currently-active event, routed through `EventScheduler.activeEvents(at:)`, sorted by priority then start date
