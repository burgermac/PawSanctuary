...
**Status (15 Aug 2026): §3.1–§3.2 done, §3.3–§3.5 not started.** `EventRegistry.activeEvents` added (commit `b023431`), routed through `EventScheduler`. `MergeBoardViewModel.activeEvent` renamed to `activeEvents: [EventDefinition]` (thin wrapper over `EventRegistry.activeEvents`) — all four call sites updated to compile against it: the two UI sites (`MergeBoardView.swift`'s `routeContent`, `PanelViews.swift`'s `TaskStripView`) now read `.activeEvents.first`, preserving today's single-card behavior (§3.4 will replace this with a real `ForEach`); the Event Pass purchase-flow fallback in `applyPurchase` now reads `activeEvents.first?.id` (same semantics as before); `claimEventMilestone` — part of the already-dead `EventProgress` system, out of scope per §6 — now reads `EventRegistry.currentEvent` directly instead of through the renamed property, so that dead path's behavior is untouched. No behavior change: today's three events never overlap, so `.first` still agrees with the old `currentEvent`/`activeEvent` values everywhere. Full suite verified at 285/285 green, 0 regressions. **Next: §3.3** — the actually load-bearing fix, `checkEventLifecycle()`'s single rider provider, which today can only track one event's rider even though `OrderRewardRegistry` beneath it already supports many. §3.4 (UI) and §4 (the overlapping test event that proves this on screen) depend on §3.3 landing first.
