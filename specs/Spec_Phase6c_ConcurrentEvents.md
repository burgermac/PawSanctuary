...
**Status (16 Aug 2026): §3.1–§3.4 done, §3.5 (no migration needed) also satisfied — only §4 remains.** `EventRegistry.activeEvents` added (commit `b023431`), routed through `EventScheduler`. `MergeBoardViewModel.activeEvent` renamed to `activeEvents: [EventDefinition]` (commit `19404b1`). **§3.3 — the load-bearing fix:** `checkEventLifecycle()`'s single `activeEventRiderProvider: EventTokenRiderProvider?` is now `activeEventRiderProviders: [String: EventTokenRiderProvider]`, diffed against `EventRegistry.activeEvents` on every check (commit `ac08fe0`) — two concurrently-active events can each register and keep their own rider, where previously only one could ever hold the slot. **§3.4 — UI wiring — is now done too (commit pending):** `TaskSheet.event` gained an associated `String` event ID (the enum could no longer be `String`-raw-value-backed once one case carries a payload, so it dropped `RawRepresentable` in favor of a hand-written `id`/`title`, both exhaustive over the event ID); `TaskStripView`'s single `if let` became `ForEach(viewModel.activeEvents) { event in ... .onTapGesture { activeSheet = .event(event.id) } }`, matching the existing `ForEach(viewModel.dailyChallenges)`/`ForEach(viewModel.activeQuests)` pattern in the same strip; `MergeBoardView.swift`'s `routeContent` now matches `.task(.event(let eventID))`, looks the ID up in `viewModel.activeEvents`, and — if the event ended in the tap-to-presentation gap — dismisses (`activeRoute = nil` via an `onAppear` on a `Color.clear` placeholder) rather than showing a stale/empty sheet. `SheetRoute.id` updated from `t.rawValue` to `t.id` to match. Verified on screen in the simulator against the real `founders_circle_aug2026` event (currently active): the task strip shows one "Event" card reading "Founders' Circle · 18d 7h left · 0/600 tokens", tapping it opens that event's own Event Pass sheet correctly. Full suite verified at 285/285 green, 0 regressions after §3.2/§3.3/§3.4. **Next: §4** — the only work left. Add one fresh overlapping test `EventDefinition` (proposed 2026-08-20 to 2026-08-24, inside Founders' Circle's still-open window) to prove two *real* concurrently-active events on screen at once — separate cards, separate sheets, independent riders — which nothing has exercised yet since today's registry still has zero genuinely overlapping events.
...
- [ ] While §4's test event overlaps Founders' Circle, both appear as separate task-strip cards simultaneously
- [ ] Tapping each card opens that event's own sheet — not the other one's, not a stale one
- [ ] Both events' order-token riders fire independently — fulfilling an order can carry tokens for either or both simultaneously; neither event's token accrual starves while the other is active
- [ ] Ending one of the two overlapping events (time-travel or a short synthetic window) leaves the other's card, sheet, and rider unaffected
- [ ] Save/load round-trips both events' independent `progressTracks` entries correctly
- [ ] No event active → empty `activeEvents`, no cards, no crash — existing single-event-inactive behavior, unchanged
- [ ] Full test suite green
- [ ] Verified on screen: two simultaneously-active event cards, both sheets independently reachable, an order carrying a rider for each
