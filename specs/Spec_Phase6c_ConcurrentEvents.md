...
**Acceptance follow-up (16 Aug 2026, commit pending):** all four previously-unchecked §5 items now have real test coverage — 6 new tests across `EventSchedulerTests`, `ProgressTrackTests`, and a new `ConcurrentEventRiderRegistrationTests` class (`LiveOpsEngineTests.swift`, `EventSystemTests.swift`), 291/291 green. Rider independence and the save/load round-trip are now fully checked. Two items remain marked `[~]` (partial) rather than `[x]`: "ending one event leaves the other unaffected" and "no event active → empty, no crash" both hit the same real architectural limit — `EventRegistry.activeEvents` is driven by the real wall clock against a hardcoded event list, not independently injectable, so their literal end-to-end scenarios (a real event actually ending mid-session; the calendar actually being empty) can't be produced on demand without either waiting for Foster Weekend's real 2026-08-18 end date or a testability refactor. Both were substituted with the strongest tests available without that refactor — see §5 for exactly what each does and doesn't cover.

Remaining before this spec can be considered fully closed: design-authority review (still DRAFT), and, if ever in doubt, closing the two `[~]` items' residual gap (most directly: a testability refactor injecting the event list/clock, or simply waiting for a natural transition and checking then).
...
- [x] Both events' order-token riders fire independently — fulfilling an order can carry tokens for either or both simultaneously; neither event's token accrual starves while the other is active — `ConcurrentEventRiderRegistrationTests.testOrderRewardRegistryFiresRidersForMultipleSimultaneouslyRegisteredEvents` proves it deterministically at the registry level; `testCheckEventLifecycleRegistersAnIndependentRiderForEachRealActiveEvent` proves the real, shipped `checkEventLifecycle()` registers one independent rider per real active event (guards on today's real overlap actually existing, doesn't silently pass once Foster Weekend's window closes)
- [~] Ending one of the two overlapping events leaves the other's card, sheet, and rider unaffected — **partially verified.** `EventSchedulerTests.testEndingOneOverlappingEventLeavesTheOtherActive` proves the underlying scheduler data source correctly drops one ID while still reporting the other (synthetic dates, deterministic); `ConcurrentEventRiderRegistrationTests.testCheckEventLifecycleIsIdempotentWhenActiveEventsUnchanged` proves the real `checkEventLifecycle()`'s "leave unchanged ones alone" branch holds. **Not verified:** the real `checkEventLifecycle()`'s "unregister a dropped rider" branch, end-to-end, against the real registry — `EventRegistry.activeEvents` isn't injectable, so exercising this for real requires either waiting past Foster Weekend's actual 2026-08-18 end date or a testability refactor, neither done here. The Set-diffing logic itself is simple and already code-reviewed.
- [x] Save/load round-trips both events' independent `progressTracks` entries correctly — `ProgressTrackTests.testTwoConcurrentEventsProgressTracksRoundTripIndependently`, using the real production registry and both real event IDs, round-tripped through actual `Codable` encode/decode (not just in-memory capture/restore)
- [~] No event active → empty `activeEvents`, no cards, no crash — **partially verified.** `EventSchedulerTests.testActiveEventsIsEmptyWhenNothingOverlapsTheQueriedDate` proves the underlying scheduler primitive returns `[]` safely for non-overlapping dates. **Not verified:** the real registry/ViewModel/UI in a genuinely empty-calendar state — not producible on screen without changing system time or the real event dates, neither attempted. Downstream code (`ForEach` over an array, `.filter`/`.sorted` on an empty collection) can't crash on empty input as a matter of Swift's type system, which is why this residual gap is judged low-risk rather than closed with a dynamic test.
- [x] Full test suite green (291/291, 6 new tests this pass, 0 regressions)
